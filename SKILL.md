...
1. `结构体用`：按动静定体用；默认动卦为用、静卦为体。
2. `现实角色`：根据领域和具体问法，判断体、用分别代表谁或什么。

先完成结构体用，再做现实角色映射。领域只提供默认参照，具体问法优先。例如：

- 感情问“对方怎么看我”：体通常对应问卦者及其状态，用通常对应对方及其反应。
- 感情问“要不要主动”：体通常对应问卦者，用通常对应行动后的关系反馈；不能直接把用当成对方内心。
- 工作问“能否跳槽”：体通常对应本人和当前工作状态，用通常对应目标岗位、公司或外部机会。
- 工作问“领导怎么看我”：体通常对应本人表现，用通常对应领导或评价机制。
- 考试、面试问“能否通过/拿到 offer”：体通常对应本人及准备、发挥，用通常对应考试、面试或录用机制。
- 健康问“病能否恢复”：体通常对应本人及身体恢复力，用通常对应病情或治疗过程；只讨论卦象中的承受、阻滞和变化，不作医学诊断。

体用关系先给作用方向，不直接生成心理标签、主动程度或成败结论。卦意负责说明“是什么局”，体用负责说明“怎么作用”；二者必须联合判断。

### 什么时候要求补充背景

- 问题和卦例字段完整、现实角色清楚时，先给方向性初断，不先盘问背景。
- 当同一卦象存在两个以上会改变主断的现实解释时，才询问背景；优先问一至两个区分度最高的问题，例如“最近是谁更主动”“是否刚发生拒绝、争执、变动或断联”。
- 用户问的是他人内心、关系状态或行为动机，而卦象只能确定“受影响、受阻、靠近、退避”等方向，不能确定具体事件时，先标明边界，再请求最小背景。
- 用户要求精确判断具体行为、时间点或现实事件，但缺少事件锚点、双方角色或关键变化时，先要求补充这些最小信息；不得用背景缺口硬编剧情。
- 若背景不影响主方向，只影响例证、强弱或时间精度，不要求补充；直接给出初断并标明条件推断。
- 用户补充背景后，重新加权卦意、体用、动变和旺衰，并明确区分“卦象直接支持”“用户提供的事实”“结合后的推断”；不得把补充背景反说成原断已准确预言。

背景问题最多两条，且必须说明它们用于区分哪两个解释。没有补充时继续完成主断，不得把“需要背景”当作停止解卦的理由。
...
4. `模型`：读取 `references/workflow.md`，按“问法与角色映射 -> 卦意主轴 -> 体用生克 -> 旺衰 -> 动爻位置 -> 变卦路径 -> 变卦落点 -> 互卦 -> 外应 -> 错综”执行。
   卦意与体用并列为主轴：本卦先定当前局面的性质，体用再说明作用方向；不要见一象断一象，也不要只凭体用关系跳到心理标签。
...
- 默认输出 Markdown。
- 先给人话结论，再展开九步依据。
- 对“能不能/会不会/是否/成不成”等二元问题，最终结论必须给出单一明确方向；不得用“有机会但”“倾向于”“可能”“取决于”“未必”等措辞回避二选一判断。
- 对“对方是什么感觉/怎么想/态度如何/关系是什么状态”等描述型问题，不强行压缩成“喜欢/不喜欢”；先给主导状态、内部机制和外显倾向，再说明是否足以推断具体心理标签。
...
- 二元问题的结论只允许一个主方向。描述型问题不套用二元结论模板，必须保留卦意、体用和背景之间的层次差异。
...
| 现实角色不清 | 领域明确但具体问法无法判断体、用分别代表谁或什么，且不同映射会改变主断 | 先给不依赖角色的卦意方向，再询问一个最小澄清问题；不得静默替换角色 |
| 背景分支过多 | 卦象方向明确，但具体心理、行为或事件存在两个以上会改变落点的现实解释 | 先给方向性初断，再最多询问两条能区分分支的背景；未补充时保留分支，不编造剧情 |
